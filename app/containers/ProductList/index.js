...
import { fetchCategoryProductListAC } from '../../redux/actions/products.action';
...
class ProductList extends Component{
  constructor(props){
    ...
    this.state = {
      categoryID: '',       // 当前一级分类ID
      categorySubID: '',    // 当前二级分类ID
      categoryThreeID: '',  // 当前三级分类ID
      productList: [],      // 商品分类数据
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      categorySubID: params.categorySubID,
      categoryThreeID: params.categoryThreeID,
    },() => {
  ...
  componentWillReceiveProps(nextProps) {
    // 商品列表
    if(!Immutable.is(this.props.$products.get('productList'),nextProps.$products.get('productList'))) {
      let productList = Immutable.Map.isMap(nextProps.$products) ?
        nextProps.$products.toJS().productList : nextProps.$products.productList;
      if('success' === productList.status){
        this.setState({
  ...
  render() {
    ...
    let { refreshing, moreLoading, pageNum, } = this.state;

    return (
      <View style={styles.container}>
        <SearchBtn
          navigation={this.props.navigation}
          onPressCenter={() => {this.props.navToSearch()}}
          type='productList'
        />
  ...
  renderSpecItem = (specItem,productItem) => {
    ...
    return (
      <View style={styles.specsWrapper} key={specItem.specID}>
        <View>
          <ChangePrice price={specItem.productPrice} suffix={specItem.saleUnitName} big={styleConsts.H3} small={styleConsts.H5} width={'auto'} fontWeight='600' />
        ...
        <ProductNum
          styles={[{marginTop: -3, marginRight: 10}]}
  ...
  fetchProductList = (pageNum,resove) => {
    let { categoryID, categorySubID, categoryThreeID, pageSize } = this.state;
  ...
  fetchMoreProductList = (resove) => {
    ...
    if(moreLoading) {
      let pageNum = Math.floor(productList.length / 20 + 1);
      this.fetchProductList(pageNum,resove);
    }
  };
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchProductList: (opts) => {
      dispatch(fetchCategoryProductListAC(opts));
    },
    // 将商品ID保存到redux中
    saveProductID: (opts) => {
