...
import HeaderWidthShopInfoAndSort from './Header';
...
import SlideSearch from './containers/SlideSearch'
...
class PurchaseList extends Component{
  ...
  render(){
    ...
    for(let key in specsObj) {
      ...
      skuNums +=  parseFloat(specsObj[key].num) > 0 ? 1 : 0;
    }
    ...
    return (
      <View style={styles.wrapper}>
          ...
          rightButtons={[
            {
              ...
              cancelCallBack:this.FilterPurchaseList
            },
            {
            ...
            <View style={styles.footer}>
              ...
              <TouchableWithoutFeedback onPress={() => totalPrice !== 0 ? this.addToCart() : {}}>
                <View style={[styles.footerRightPart, totalPrice !== 0 ? {} : { backgroundColor: styleConsts.mainColorOpacity }]}>
                  <Text style={styles.footBtnTxt}>加入进货单{skuNums ? `（${skuNums}）` : ''}</Text>
                </View>
        ...
        <SlideSearch width={width-40} ref={ (ref) => this.slide = ref }/>
      </View>
    )
  }
  _renderStickyView = () => {
  ...
  renderSpecs = (spec,specIndex,index,productID) => {
    ...
    return (
      <View style={styles.specsItem} key={spec.productSpecID}>
        <View>
          <ChangePrice price={spec.productPrice} big={styleConsts.H3} small={styleConsts.H5} />
          <Text style={styles.specTxt}>{spec.specContent}/{spec.saleUnitName}</Text>
        </View>
        <TextInput
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    // // 筛选采购清单
    // navToFilterPurchaseList: () => {
    //   dispatch({type: NAV_TO_FILTER_PURCHASE_LIST});
    // }
  }
};
